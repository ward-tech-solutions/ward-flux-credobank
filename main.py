...
@app.post("/api/v1/diagnostics/mtr/store")
async def store_mtr_data(
    ...
    from database import MTRResult
...
@app.get("/api/v1/diagnostics/mtr/history/{ip}")
async def get_mtr_history(
    ...
    results = db.query(MTRResult)\
        .filter(
            MTRResult.device_ip == ip,
            MTRResult.timestamp >= since
        )\
...
@app.get("/api/v1/diagnostics/trends/{ip}")
...
@app.post("/api/v1/diagnostics/bulk/ping")
async def bulk_ping_devices(
    ...
    db.commit()
    return {'results': results, 'total': len(device_ips), 'successful': len([r for r in results if r.get('is_reachable')])}
...
async def export_diagnostic_results(
    ...
    if format == 'csv':
        ...
        writer.writerow(['Device IP', 'Device Name', 'Packet Loss %', 'Avg Latency (ms)', 'Min Latency (ms)', 'Max Latency (ms)', 'Timestamp'])
        ...
        return StreamingResponse(
            ...
            media_type='text/csv',
            headers={'Content-Disposition': f'attachment; filename=diagnostics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'}
        )
...
@app.post("/api/v1/diagnostics/dns/lookup")
...
@app.post("/api/v1/diagnostics/dns/reverse")
...
@app.post("/api/v1/diagnostics/portscan")
async def port_scan(
    ip: str,
...
@app.post("/api/v1/diagnostics/baseline/calculate")
async def calculate_baseline(
    ...
    from database import PingResult, PerformanceBaseline
    from sqlalchemy import func
    from datetime import timedelta
...
@app.get("/api/v1/diagnostics/baseline/{ip}")
async def get_baseline(
    ...
    from database import PerformanceBaseline
...
@app.post("/api/v1/diagnostics/anomaly/detect")
