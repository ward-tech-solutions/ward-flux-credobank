...
def _get_standalone_devices(
    ...
    for device in devices:
        ...
        payload.append({
            ...
            "is_flapping": device.is_flapping if hasattr(device, 'is_flapping') else False,
            "flap_count": device.flap_count if hasattr(device, 'flap_count') else 0,
            "flapping_since": device.flapping_since.replace(tzinfo=timezone.utc).isoformat() if hasattr(device, 'flapping_since') and device.flapping_since else None,
