#!/usr/bin/env python3
"""Test SNMP credential decryption"""
import sys
...
try:
    ...
    async def test_plaintext():
        ...
        return result.success, result.value if result.success else result.error

    success, msg = asyncio.run(test_plaintext())
    if success:
        print("✅ The value IS plaintext - SNMP works with it directly!")
        print(f"Community string: {cred.community_encrypted}")
    else:
        print(f"❌ Not plaintext either - SNMP error: {msg}")
